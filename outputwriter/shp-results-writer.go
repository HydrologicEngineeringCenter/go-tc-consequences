...
func InitNHCShpResultsWriter_Projected(filepath string, layerName string, ESPG int) (*shpResultsWriter, error) {
...
func InitNHCShpResultsWriter(filepath string, layerName string) (*shpResultsWriter, error) {
	return InitNHCShpResultsWriter_Projected(filepath, layerName, 4326)
}
func (srw *shpResultsWriter) Write(r consequences.Result) {
		...
		for i, val := range r.Headers {
			...
			func() {
				if val == "hazard" { //not a huge fan of this, because it is specific to that kind of hazard.
					fieldDef := gdal.CreateFieldDefinition("hazard", gdal.FT_String)
					defer fieldDef.Destroy()
	...
	for i, val := range r.Headers {
		...
		valType := att.Kind()
		if val == "hazard" { //not a big fan of this, very hazard specific.
